...
impl LoggingConfig {
    ...
    pub fn from_env() -> Self {
        Self {
            level: std::env::var("KROKIT_LOG_LEVEL").unwrap_or_else(|_| "off".to_string()),
            file_path: std::env::var("KROKIT_LOG_FILE").ok().map(PathBuf::from),
            include_spans: std::env::var("KROKIT_LOG_SPANS").map(|v| v == "true").unwrap_or(false),
            json_format: std::env::var("KROKIT_LOG_JSON").map(|v| v == "true").unwrap_or(false),
        }
    }
