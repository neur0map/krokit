...
impl KrokitPtyManager {
    ...
    fn setup_child_process(&self, shell: Shell, quiet: bool) -> ! {
        unsafe {
            ...
            // Set KROKIT_SESSION_ID environment variable
            let session_env = std::ffi::CString::new("KROKIT_SESSION_ID").unwrap();
...
impl Drop for KrokitPtyManager {
    fn drop(&mut self) {
...
mod tests {
    ...
    fn test_pty_manager_creation() {
        let pty = KrokitPtyManager::new().unwrap();
    ...
    fn test_unique_session_ids() {
        let pty1 = KrokitPtyManager::new().unwrap();
        thread::sleep(Duration::from_millis(1)); // Ensure different timestamp
        let pty2 = KrokitPtyManager::new().unwrap();
    ...
    fn test_create_pty_pair() {
        let result = KrokitPtyManager::create_pty_pair();
        assert!(result.is_ok());
    ...
    fn test_pty_pair_communication() {
        let (master_fd, slave_fd) = KrokitPtyManager::create_pty_pair().unwrap();
